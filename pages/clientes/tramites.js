...
import { TRAMITES, FORM_BUSQUEDA_CLIENTE_TRAMITE } from '../../constants/form'
...
function Tramitestodos() {
  ...
  const [value, setValue] = useState(TRAMITES)
  const [valueModal, setValueModal] = useState(TRAMITES)
  ...
  const handleGetters = async () => {
    ...
    await axios.post('/api/tramites/allTramiteSum').then(res => {
      if (res.status === 200) {
        setTotales(res.data?.tramites[0])
      }
    })
  ...
  const columns = [
    ...
    {
      field: 'Deuda', headerName: 'Deuda', flex: 0.1, renderCell: (rows) => {
        return (
          <Stack direction="row" spacing={2} >
            $ {rows.row.Deuda == rows.row.MontoTotal ? 0 : rows.row.Deuda}
          </Stack>
        );
      },
    },
  ...
  const handleBucar = async () => {
    var data = {
      Cliente: value.Cliente,
      Ingreso: value.Ingreso,
      Egreso: value.Egreso,
      ConDeuda: value.ConDeuda,
      Retirados: value.Retirados
    }
    await axios.post('/api/tramites/allTramite', data).then(res => {
      if (res.status === 200) {
        var rowsR = []
        res.data?.tramites.map((item) => {
          rowsR.push({
    ...
    await axios.post('/api/tramites/allTramiteSum', data).then(res => {
      if (res.status === 200) {
        setTotales(res.data?.tramites[0])
  ...
  return (
    <div>
      <Title titulo='Busqueda por cliente' />
      <Busqueda ocultar={ocultar} setOcultar={setOcultar} title={'Buscar'} options={options} constForm={FORM_BUSQUEDA_CLIENTE_TRAMITE} value={value} setValue={setValue} handleCancel={handleCancel} handleBucar={handleBucar} />
      ...
      <div style={{margin: '30px', float: 'right'}}>
        <h3>Pago Parcial: ${totales.parcial? parseInt(totales.parcial) : 0}</h3>
        <h3>Deuda Parcial: ${totales.parcial > 0 ? parseInt(totales.total - totales.parcial) : 0}</h3>
        <h3>Total: ${totales.total? parseInt(totales.total) : 0}</h3>
      </div>
      <Modal
        open={open}
        onClose={handleClose}
